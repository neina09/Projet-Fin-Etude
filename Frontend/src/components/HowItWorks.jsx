...
function HowItWorks() {
  ...
  const stepsCopy = t("howItWorks.steps", [])
  const steps = [
    { icon: Search, ...stepsCopy[0] },
    { icon: Users, ...stepsCopy[1] },
    { icon: CheckCircle, ...stepsCopy[2] }
  ]

  return (
    <section id="how-it-works" className="bg-white py-24" dir={dir}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-12">
        <div className="mx-auto mb-16 max-w-2xl text-center">
          <h2 className="text-3xl font-black text-slate-950 md:text-5xl">{t("howItWorks.title")}</h2>
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-3 lg:gap-12">
          {steps.map((step, index) => {
            ...
            return <StepCard key={step.title} step={step} index={index} Icon={Icon} />
          })}
        </div>
      </div>
    </section>
    ...
    <div className="group flex flex-col items-center text-center">
      <div
        ...
        style={{
          ...
          boxShadow: hovered ? "0 20px 25px -5px rgba(29,78,216,0.2)" : "0 1px 2px 0 rgba(0,0,0,0.05)"
        }}
